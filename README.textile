...
It can be embedded in any AceAddon addons (including modules) and automatically disable all addon overlays when the addon is disabled. Frames can be (un)locked individually or all at once.

When adding the proper methods to the target frame, LibMovable-1.0 add some options to enable and disable it.
